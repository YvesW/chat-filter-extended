...
//Chat tab blinking, once. At the bottom, you see 179 a couple times cause I had to click on the tab to make it not solid anymore, but the interesting part there is 183's instStack[1] going to 0 (aka the VarCInt).
//183 proc,chat_alert_set => intStack[1] = starts at 249 when flashing, 1 is substracted every client tick (probably client tick), then it stops at 25 when it's solid instead of flashing and when you click on the button to remove the flash, it goes to 0. This corresponds with VarClientInt 44, 45, 46, 438, 47, 48 for game chat, public chat, private chat, FC, CC, trade chat respectively (see CS2 code). Also procs one when clicking on a chattab! intStack[0] likely gives the tab that is flashing/solid/stopped flashing.
//179 proc,chat_alert_get => procs every clienttick as well, but also at random times or when hovering over the chattab. intStack[0] likely gives you the chat tab it is checking. No interesting details.
//180 proc,chat_alert_enable => only procs once to enable the flash. Changing the VarClientInt in PostScriptFired works to e.g. completely disable the flash, make it shorter or make it solid. Does not proc when clicking on a chattab. intStack[0] = e.g. 2 for public, 6 for trade. Get the IntStack in ScriptPreFired though ofc, not in ScriptPostFired
//182 clientscript,chat_alert_flash => also procs every clienttick, has a scriptevent but not that interesting probably. Does not proc when clicking on tabs. The CS2 code might be semi-interesting since it might explain the 25 from script 183.
/*
180 intStackSize = 1
180 intStack[0] = 2
180 stringStackSize = 0
180 scriptEvent = null
179 intStackSize = 1
179 intStack[0] = 2
179 stringStackSize = 0
179 scriptEvent = null
183 intStackSize = 2
183 intStack[0] = 2
183 intStack[1] = 249
183 stringStackSize = 0
183 scriptEvent = null
182 intStackSize = 0
182 stringStackSize = 0
182 source = nn@1944e57a
182 WidgetUtil.componentToInterface(scriptSourceId) = 162
182 WidgetUtil.componentToId(scriptSourceId) = 12
182 scriptSource.getIndex() = -1
Arg length = 2
182
2
Done printing script arguments.
182 scriptEvent.getOp() = 0
182 scriptEvent.getOpbase() = null
182 scriptEvent.getTypedKeyChar() = 0
182 scriptEvent.getTypedKeyCode() = 0
179 intStackSize = 1
179 intStack[0] = 2
179 stringStackSize = 0
179 scriptEvent = null
